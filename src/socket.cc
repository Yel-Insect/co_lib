...
Socket::ptr Socket::CreateTCP(co_lib::Address::ptr address) {
    Socket::ptr sock(new Socket(address->getFamily(), TCP, 0));
    return sock;
}

Socket::ptr Socket::CreateUDP(co_lib::Address::ptr address) {
...
Socket::ptr Socket::accept() {
    ...
    if (newsock == -1) {
        LOG_ERROR(g_logger) << "accept(" << m_sock << ") error"
...
bool Socket::bind(const Address::ptr addr) {
    if (!isValid()) {
        newSock();
        if (CO_LIB_UNLICKLY(!isValid())) {
            return false;
        }
    }

    if (CO_LIB_UNLICKLY(addr->getFamily() != m_family)) {
...
Address::ptr Socket::getRemoteAddress() {
    ...
    if (getpeername(m_sock, result->getAddr(), &addrlen)) {
        ...
        return Address::ptr(new IPv4Address(m_family));
    }
...
bool Socket::cancelRead() {
    return IOManager::GetThis()->cancelEvent(m_sock, co_lib::IOManager::READ);
}

bool Socket::cancelWrite() {
    return IOManager::GetThis()->cancelEvent(m_sock, co_lib::IOManager::WRITE);
}
