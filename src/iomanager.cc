...
int IOManager::addEvent(int fd, Event event, std::function<void()> cb) {
    ...
    if (fd_ctx->events & event) {
        ...
        CO_LIB_ASSERT(!(fd_ctx->events & event));
    }
    ...
    CO_LIB_ASSERT(!event_ctx.scheduler && !event_ctx.fiber && !event_ctx.cb);
    ...
    if (cb) {
        ...
        CO_LIB_ASSERT2(event_ctx.fiber->getState() == Fiber::EXEC
        ...
        for (int i = 0; i < rt; i++) {
            ...
            if (event.data.fd == m_tickleFds[0]) {
                ...
                while (read(m_tickleFds[0], &dummy, 1) == 1) {
                    // LOG_DEBUG(g_logger) << "reading pipe";
                };
                continue;
            }
