...
ByteArray::Node::Node(size_t mSize, Node *next)
	: m_next(next)
	, m_size(mSize) {
	m_ptr = new char[mSize];
}

ByteArray::Node::~Node() { delete[] m_ptr; }

ByteArray::ByteArray(size_t mBaseSize, size_t mCapacity)
...
void ByteArray::updateCapacity(size_t size) {
...
size_t ByteArray::getCapacity() const { return m_capacity; }

size_t ByteArray::getDataSize() const { return m_data_size; }

size_t ByteArray::getBaseSize() const { return m_base_size; }

size_t ByteArray::write(const char *buf, int size) {
	updateCapacity(size);
	size_t w_size = 0;
	while (size > 0)
	{
		CO_LIB_ASSERT(m_write_node);
		if (m_base_size - m_write_index > size)
		{
			if (buf) {
				memcpy(m_write_node->m_ptr + m_write_index, buf + w_size, size);
			}
			m_write_index += size;
			w_size += size;
			size = 0;
			...
			m_write_index = 0;
			CO_LIB_ASSERT(m_write_node->m_next);
...
size_t ByteArray::read(char *buf, int size) {
	size = m_data_size > size ? size : m_data_size;
	size_t r_size = 0;
	while (size > 0) {
		if (m_base_size - m_read_index > size) {
			if (buf) {
				memcpy(buf + r_size, m_read_node->m_ptr + m_read_index, size);
			}
			m_read_index += size;
			r_size += size;
			size = 0;
...
void ByteArray::write(const std::string &buf) {
	write(buf.c_str(), buf.size());
}

const std::string ByteArray::read(size_t size) {
	std::shared_ptr<char> buf(new char[size + 1], [](auto &a)
							  { delete[] a; });
	read(buf.get(), size);
	return std::string(buf.get(), 0, size);
}

size_t ByteArray::getWriteBuffers(std::vector<iovec> &buffers,
...
size_t ByteArray::getReadBuffers(std::vector<iovec> &buffers,
								 size_t size, bool is_update) {
	size = m_data_size > size ? size : m_data_size;
	size_t r_size = size;
...
void ByteArray::deallocNode(ByteArray::Node *node) {
	node->m_next = nullptr;
