...
// void test_yaml() {
// 	YAML::Node root = YAML::LoadFile("/home/beautiful/桌面/server/sylar/test.yml");
// 	print_yaml(root, 0);
// }
...
#define XX_M(g_var, name, prefix) \
	...
	auto& v = g_var->getValue(); \
	for (auto& i : v) {			\
		SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << #name " " #prefix ": {" \
				<< i.first << " - " << i.second << "}"; \
	}	\
	SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << #prefix " " #name " yaml: " << g_var->toString(); \
	}
...
#define XX_PM(g_var, prefix) \
	...
	g_person->addListener(10, [](const Person& old_value, const Person& new_value) {
		SYLAR_LOG_INFO(SYLAR_LOG_ROOT()) << "before: " << old_value.toString()
